...
use crate::commands::utils::shared::fill_pixel_normalised;
...
pub fn preprocess_detection(img: &DynamicImage) -> (Array<f32, IxDyn>, f32, f32) {
    ...
    let tensor = Array::zeros((1, 3, DET_INPUT_SIZE as usize, DET_INPUT_SIZE as usize));
    let mut tensor = tensor.into_dyn();

    for y in 0..rh {
        for x in 0..rw {
            let pixel = resized.get_pixel(x, y);
            fill_pixel_normalised(&mut tensor, x, y, &pixel, MEAN, STD);
        }
    }

    (tensor, new_w as f32, new_h as f32)
}
...
pub fn preprocess_recognition(crop: &DynamicImage) -> Array<f32, IxDyn> {
    ...
    let tensor = Array::zeros((1, 3, REC_HEIGHT as usize, rw as usize));
    let mut tensor = tensor.into_dyn();
